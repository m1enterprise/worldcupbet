...
import MatchCard from "../components/MatchCardMy";
import BonusCard from "../components/BonusCard";

import { getSession } from "../lib/auth";
import { useMyBets } from "../hooks/useMyBets";
import { LoaderPinwheel } from "lucide-react";
import MatchCardAll from "../components/MatchCardAll";
...
export default function MyBets() {
  ...
  const {
    ...
    upMatch,
    upMatchAllBets
  } = useMyBets(session.id);

  // console.log(123, allBets)
  // console.log(1234, matches)
  ...
  const matchesSort = matches.filter(item => {
    const date = new Date(item.utcDate); // UTC -> lokalny czas PL
    date.setHours(0, 0, 0, 0);

    // console.log(100, date, today)

    return date <= today;
  });
  matchesSort.sort((a, b) => {
    return new Date(b.utcDate) - new Date(a.utcDate);
  });
  // console.log(0.004, matchesSort)
  ...
  const allUsersBetsObjectByMatchId = allBets.reduce((acc, item) => {
    const { matchId } = item;

    acc[matchId] ??= [];
    acc[matchId].push(item);

    return acc;
  }, {});
  // console.log("MAP_BETS", allUsersBetsObjectByMatchId)
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-2xl mx-auto p-4 pb-24">
        ...
        <div className="flex bg-muted rounded-xl p-1 gap-1 mb-4">
          <button
            className={`flex-1 h-9 rounded-lg text-sm font-semibold transition-all ${tab === "all" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            onClick={() =>
              setTab("all")
            }
          >
          ...
          <button
            className={`flex-1 h-9 rounded-lg text-sm font-semibold transition-all ${tab === "bets" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            onClick={() =>
              setTab("bets")
            }
          >
          ...
          <button
            className={`flex-1 h-9 rounded-lg text-sm font-semibold transition-all ${tab === "bonus" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
            onClick={() =>
              setTab("bonus")
            }
          >
        ...
        <div className="space-y-3">
          ...
          {tab === "all" &&
            ...
            //       key={upMatch?.id}
            //       match={upMatch}
            //       allBetsData={upMatchAllBets}
            //   />

            matchesSort.map(matchItem => (
              <MatchCardAll
                key={matchItem.id}
                match={matchItem}
                matchAllBets={allUsersBetsObjectByMatchId[Number(matchItem.id)]}
              />
          ))}
...
// function MatchCard({ match, fetchedBetData, allBetsData}) {
...
//     const currentMatchAllBets = allBetsData?.filter(bet=> String(bet?.matchId) === String(match?.id))
//     currentMatchAllBets?.map(userBet=>{
//         const userPointsInfo = calcMatchPoints(userBet, match)
//         userBet.pointsInfo = userPointsInfo
//     })

//     console.log(currentMatchAllBets)
//      const usernameById = {
//       14: "Sud",
//       15: "Cezar",
//       16: "Jose",
...
//         <div className="px-4 pb-3">
//         {currentMatchAllBets?.map(userBet=>(
//           <div key={userBet.id} className={`mb-2 bg-muted/50 rounded-lg px-3 py-2 text-xs flex items-center justify-between bg-gradient-to-r from-white/10 via-white/10 ${
//             !isFinished
//               ? 'to-gray-100'
//               : userBet?.pointsInfo?.isExact
//                 ? 'to-yellow-100'
//                 : userBet?.pointsInfo?.isCorrect
//                 ? 'to-green-100'
//                 : 'to-red-100'
//             }`}>
//               <span className="text-muted-foreground">BET USERA {usernameById[userBet?.userId]}</span>
...
//   const [allMatchesPlayed, setAllMatchesPlayed] = useState([])


//   const tabs = [
//     {key: "bets", label: "Moje Bety"},
//     {key: "allBets", label: "Wszystkie Bety"},
...
//       // BETS BY USER ID
//       const bet_data_us = await getBetsByUserId(session?.id);
//       const bet_data = [...bet_data_us]?.sort((a,b)=>
//         Number(b?.id) - Number(a?.id)
//       )
//       if (!bet_data) return console.log('No user_bet data found');

//       console.log(315, bet_data)
...
//         const allMatchesData = data?.filter(match=>match.status === "TIMED")
//         setAllMatches(allMatchesData)

//         // ALL MATCHES, SEE BET HISTORY
//         const allMatchesPlayed = data?.filter(match=>match.status !== "TIMED")
//         setAllMatchesPlayed(allMatchesPlayed)
//         console.log(351, allMatchesPlayed)

//         const sorted = [...data].sort(
//           (a, b) => (new Date(a.utcDate).getTime()) - (new Date(b.utcDate).getTime())
//         );
        
//         const sortedNewest = [...sorted].sort(
...
//         const all_bets_filter = all_bets.filter(bet=>bet.status !== "TIMED")
//         setAllBets(all_bets_filter)
...
//             <h2 className="font-display text-base font-bold mb-3 flex items-center gap-2">
//               <Target className="w-5 h-5 text-primary" />
//               Wszystkie bety ({userBets?.length})
//             </h2> 
